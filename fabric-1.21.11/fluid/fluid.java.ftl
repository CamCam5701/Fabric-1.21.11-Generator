...
public abstract class ${name}Fluid extends FlowingFluid {
	...
	@Environment(EnvType.CLIENT) public static void clientLoad() {
		final Identifier stillTexture = Identifier.parse("${data.textureStill.format("%s:block/%s")}");
		final Identifier flowingTexture = Identifier.parse("${data.textureFlowing.format("%s:block/%s")}");
		final Identifier overlayTexture = <#if data.textureRenderOverlay?has_content>Identifier.parse("${data.textureRenderOverlay.format("%s:textures/%s")}.png")<#else>null</#if>;
